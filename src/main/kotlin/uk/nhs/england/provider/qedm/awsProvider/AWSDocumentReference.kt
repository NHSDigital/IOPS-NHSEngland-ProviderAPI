...
class AWSDocumentReference(val messageProperties: MessageProperties, val awsClient: IGenericClient,
    ...
    fun createUpdateAWSDocumentReference(newDocumentReference: DocumentReference, bundle: Bundle?): DocumentReference? {
        ...
        if (newDocumentReference.hasCustodian() && newDocumentReference.getCustodian().hasIdentifier()) {
            val organisation = awsOrganization.getOrganization(newDocumentReference.custodian.identifier)
        ...
        for (participant in newDocumentReference.author) {
            if (participant.hasIdentifier()) {
                if (participant.identifier.system.equals(uk.nhs.england.provider.qedm.util.FhirSystems.NHS_GMC_NUMBER)||
                    participant.identifier.system.equals(uk.nhs.england.provider.qedm.util.FhirSystems.NHS_GMC_NUMBER)) {
                    val dr = awsPractitioner.getPractitioner(participant.identifier)
                    if (dr != null) {
                        awsBundleProvider.updateReference(participant,dr.identifierFirstRep,dr)
                    } else if (participant.identifier.system.equals(uk.nhs.england.provider.qedm.util.FhirSystems.ODS_CODE)) {
                        val organisation = awsOrganization.getOrganization(participant.identifier)
