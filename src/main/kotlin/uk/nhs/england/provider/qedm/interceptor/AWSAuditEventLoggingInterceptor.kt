...
)
 {
    ...
    fun createAudit(
        ...
        if (httpRequest.method == "GET") {
            auditEvent.type = Coding().setSystem(uk.nhs.england.provider.qedm.util.FhirSystems.ISO_EHR_EVENTS).setCode("access")
        } else {
            auditEvent.type = Coding().setSystem(uk.nhs.england.provider.qedm.util.FhirSystems.ISO_EHR_EVENTS).setCode("transmit")
           // DISABLED 3/Oct/2022 Don't put resources in audit  entityComponent.addDetail().setType("resource").value = StringType(resource)
        }
        entityComponent.type = Coding().setSystem(uk.nhs.england.provider.qedm.util.FhirSystems.FHIR_RESOURCE_TYPE).setCode(fhirResourceName)
        ...
        agentComponent.type = CodeableConcept(Coding().setSystem(uk.nhs.england.provider.qedm.util.FhirSystems.DICOM_AUDIT_ROLES).setCode("110150"))
        ...
        if (patientId != null) {
            ...
            patient.type =
                CodeableConcept(Coding().setSystem(uk.nhs.england.provider.qedm.util.FhirSystems.V3_ROLE_CLASS).setCode("PAT"))
            if (patientId.startsWith("Patient/")) {
                patient.who = Reference().setReference(patientId).setType("Patient")
            } else {
                patient.who =
                    Reference().setType("Patient")
                        .setIdentifier(Identifier().setSystem(uk.nhs.england.provider.qedm.util.FhirSystems.EMIS_PATIENT_IDENTIFIER).setValue(patientId))
            }
        }
